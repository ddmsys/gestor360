    ...
    >
      ...
      <div className="relative bg-white rounded-[var(--radius-xl)] shadow-[var(--shadow-md)] p-6 w-full max-w-sm mx-4 z-10">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-[var(--color-text-title)] text-sm">Remover seção</h3>
...
function SortableItem({
  section, idx, total, isSelected,
  onSelect, onMove, onToggleVisible, onDuplicate, onRequestDelete,
}: SortableItemProps) {
  const {
    attributes,
    listeners,
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3 py-2.5">
          ...
          <button
            ...
            className="text-[var(--color-border)] hover:text-[var(--color-text-muted)] shrink-0 cursor-grab active:cursor-grabbing touch-none"
          ...
          {!section.visible && (
            <span className="text-[10px] text-[var(--color-text-muted)] shrink-0">oculta</span>
          )}

          {/* Botões ▲▼ (fallback de acessibilidade) */}
            ...
            <ul className="flex-1 overflow-y-auto py-2 space-y-1 px-2">
              {sections.length === 0 && (
                <li className="text-center py-8">
                  <p className="text-xs text-[var(--color-text-muted)]">Nenhuma seção ainda.</p>
                  <button
                    type="button"
                    onClick={onAdd}
