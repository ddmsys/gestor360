...
export function SectionRendererPreview({ section }: Props) {
...
function renderInner(section: PageSection, hasBg = false) {
  switch (section.type) {
    ...
    case 'ferramentas':
      return (
        <div className="py-16 flex items-center justify-center bg-[var(--color-bg-canvas)]">
          <p className="text-sm text-[var(--color-text-muted)] italic">
            Seção Ferramentas — visualização disponível no site público
          </p>
        </div>
      )
    default:
